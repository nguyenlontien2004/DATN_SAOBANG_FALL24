                        ...
                        @foreach ($suatChieus as $suatChieu)
                            <tr>
                               ...
                               <td class="text-center">
                                    <form action="{{ route('suatchieu.restore', $suatChieu->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('PATCH')
                                        <button class="btn btn-success btn-sm">Khôi phục</button>
                                    </form>

                                    <form action="{{ route('suatchieu.forceDelete', $suatChieu->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa vĩnh viễn mục này không?');">
                                        @csrf
                                        @method('DELETE')
