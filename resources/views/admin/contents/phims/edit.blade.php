        ...
        <div class="card-body">
            <form action="{{ route('phim.update', $phim->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="ten_phim" name="ten_phim" value="{{ old('ten_phim', $phim->ten_phim) }}" required>
                </div>
                ...
                <div class="mb-3 row">
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="thoi_luong" name="thoi_luong" value="{{ old('thoi_luong', $phim->thoi_luong) }}" required>
                    </div>
                    <div class="col-md-6">
                ...
                <div class="mb-3 row">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="ngay_khoi_chieu" name="ngay_khoi_chieu" value="{{ old('ngay_khoi_chieu', $phim->ngay_khoi_chieu) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label for="ngay_ket_thuc" class="form-label">Ngày Kết Thúc</label>
                        <input type="date" class="form-control" id="ngay_ket_thuc" name="ngay_ket_thuc" value="{{ old('ngay_ket_thuc', $phim->ngay_ket_thuc) }}">
                    </div>
                </div>

                <div class="mb-3">
                    <label for="trailer" class="form-label">Trailer (link)</label>
                    <input type="url" class="form-control" id="trailer" name="trailer" value="{{ old('trailer', $phim->trailer) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="the_loai_phim_ids[]" class="form-control" multiple required>
                        ...
                        <option value="{{ $theLoai->id }}" {{ in_array($theLoai->id, $phim->theLoaiPhims->pluck('id')->toArray() ?? []) ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="dao_dien_ids[]" class="form-control" multiple required>
                                @foreach ($daoDiens as $daoDien)
                                    <option value="{{ $daoDien->id }}"   {{ in_array($daoDien->id, $phim->daoDiens->pluck('id')->toArray() ?? []) ? 'selected' : '' }} >
                        ...
                        <div class="form-group">
                            ...
                            <select id="dien_vien_ids" name="dien_vien_ids[]" class="form-control" multiple required>
                                @foreach ($dienViens as $dienVien)
                                    <option value="{{ $dienVien->id }}" {{ in_array($dienVien->id, $phim->dienViens->pluck('id')->toArray() ?? []) ? 'selected' : '' }}>
