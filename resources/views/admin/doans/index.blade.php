                                                ...
                                                <td class="d-flex">
                                                    @if($item->deleted_at == null)
                                                        <a class="btn btn-warning"
                                                            href="{{ route('do-an.edit', $item->id) }}">Sửa</a>

                                                        <form action="{{ route('do-an.destroy', $item->id) }}" method="post"
                                                            class="d-inline"
                                                            onsubmit="return confirm('Bạn có chắc chắn không?')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class=" btn border-0 bg-white btn-danger ms-1">
                                                                Ẩn
                                                            </button>
                                                        </form>
                                                    @else
                                                        <form action="{{ route('do-an.restore', $item->id) }}"
                                                            onsubmit="return confirm('Bạn có chắc chắn không?')" method="post">
                                                            @csrf
                                                            @method('PATCH')
                                                            <button type="submit" class=" btn border-0 bg-white btn-success">
                                                                Khôi phục
                                                            </button>
                                                        </form>
                                                        <form action="{{ route('do-an.forceDelete',$item->id) }}"
                                                            onsubmit="return confirm('Bạn có chắc chắn muốn xoá đồ ăn này không?')"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
