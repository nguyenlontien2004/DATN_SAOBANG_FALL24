...
<div class="container mt-5">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('nhanvienphim.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-3">
                    <label for="ten_phim" class="form-label">Tên Phim</label>
                    <input type="text" class="form-control" id="ten_phim" name="ten_phim" value="{{ old('ten_phim') }}">
                    @error('ten_phim')
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="ngay_khoi_chieu" name="ngay_khoi_chieu" value="">
        ...
        <div class="mb-3 row">
            ...
            <input hidden type="number" class="form-control" id="luot_xem_phim" name="luot_xem_phim" value="0">
            @error('luot_xem_phim')
                <div class="text-danger">{{ $message }}</div>
            @enderror
            <div class="col-md-4">
        ...
        <div class="mb-3">
            ...
            <input type="url" class="form-control" id="trailer" name="trailer" value="{{ old('trailer') }}">
            @error('trailer')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
                ...
                <div class="form-group">
                    ...
                    <select name="the_loai_phim_ids[]" class="form-control" multiple>
                        @foreach ($theLoais as $theLoai)
                            <option value="{{ $theLoai->id }}" {{ in_array($theLoai->id, old('the_loai_phim_ids', [])) ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="dao_dien_ids[]" class="form-control" multiple>
                        @foreach ($daoDiens as $daoDien)
                            <option value="{{ $daoDien->id }}" {{ in_array($daoDien->id, old('dao_dien_ids', [])) ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="dien_vien_ids[]" class="form-control" multiple>
                        @foreach ($dienViens as $dienVien)
                            <option value="{{ $dienVien->id }}" {{ in_array($dienVien->id, old('dien_vien_ids', [])) ? 'selected' : '' }}>
        ...
        <div class="ms-3 mb-3" >
            ...
            <a href="{{ route('nhanvienphim.index') }}" class="btn btn-secondary">Quay lại</a>
        </div>
        </form>
    </div>
</div>
