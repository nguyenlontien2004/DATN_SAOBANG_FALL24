...
<div class="card container mt-5">
    <div class="card-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <a href="{{ route('nhanvienphim.create') }}">Thêm mới Phim</a>
            <span class="px-2">/</span>
            <a href="{{ route('nhanvien.phim.listSoftDelete') }}">Các mục đã xoá mềm</a>
        </div>
        <div>
                <form action="{{ route('nhanvienphim.index') }}" method="GET" class="form-inline d-flex align-items-center">
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('nhanvienphim.index') }}" id="filterForm" class="mb-4">
            <div class="row">
                <div class="col-md-4">
                    ...
                    <select name="the_loai" id="the_loai" class="form-select" style="width: 200px"
                        ...
                        @foreach ($theLoaiPhims as $theLoai)
                            <option value="{{ $theLoai->id }}" {{ request('the_loai') == $theLoai->id ? 'selected' : '' }}>
                ...
                @forelse ($phims as $index => $phim)
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <form action="{{ route('nhanvien.phim.softDelete', $phim->id) }}" method="POST"
                                    ...
                                    <a class="btn btn-warning btn-sm" href="{{ route('nhanvienphim.edit', $phim->id) }}">
                                        Sửa
                                    </a>
                                <a class="btn btn-info btn-sm" href="{{ route('nhanvienphim.show', $phim->id) }}">Xem</a>
                            </div>
                        </td>
                    </tr>
                @empty
