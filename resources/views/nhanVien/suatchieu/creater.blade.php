...
<div class="container mt-3">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('nhanvienSuatchieu.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                    <!-- Phòng Chiếu -->
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="phong_chieu_id" name="phong_chieu_id" required>
                            @foreach ($phongChieus as $phongChieu)
                                <option value="{{ $phongChieu->id }}">{{ $phongChieu->ten_phong_chieu.' - '.'Rạp: '.$phongChieu->rap->ten_rap }}</option>
                            @endforeach
                        </select>
                ...
                <a href="{{ route('nhanvienSuatchieu.index') }}" class="btn btn-secondary">Quay lại</a>
            </form>
        </div>
    </div>
</div>
