...
<div class="container mt-5">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('nhanvienSuatchieu.update', $suatChieu->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="phong_chieu_id" name="phong_chieu_id">
                            @foreach ($phongChieus as $phongChieu)
                                <option value="{{ $phongChieu->id }}" {{ $suatChieu->phong_chieu_id == $phongChieu->id ? 'selected' : '' }}>
                                    {{ $phongChieu->ten_phong_chieu.' - '.'Rạp: '.$phongChieu->rap->ten_rap  }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 row">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" id="ngay" name="ngay" value="{{ $suatChieu->ngay}}">
                    </div>
                    <div class="col-md-6">
                        <label for="ngay" class="form-label">gia</label>
                        <input type="number" class="form-control" id="gia" name="gia" value="{{ $suatChieu->gia }}">
                        @error('gia')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
