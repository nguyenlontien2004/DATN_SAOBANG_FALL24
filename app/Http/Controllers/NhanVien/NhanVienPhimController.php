...
use App\Http\Requests\NhanvienPhimRequest;
use App\Models\MaGiamGia;
use Illuminate\Support\Facades\DB;

class NhanVienPhimController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('nhanVien.phims.index', compact('phims', 'theLoaiPhims', 'theLoaiId', 'searchQuery'));
    }

    public function show($id)
    {
        $phim = Phim::with('daoDiens', 'dienViens', 'theLoaiPhims')->findOrFail($id);
        return view('nhanVien.phims.show', compact('phim'));
    }
    public function create()
    {
        ...
        return view('nhanVien.phims.creater', compact('daoDiens', 'dienViens', 'theLoais'));
    }
    ...
    public function store(Request $request)
    {
       ...
       DB::rollBack();
        ...
        return redirect()->route('nhanvienphim.index')->with('success', 'Phim đã được thêm thành công.');
    }
    ...
    public function edit(Phim $nhanvienphim)
    {
        $phim = $nhanvienphim;
        $daoDiens = DaoDien::where('trang_thai', 1)->get();
        $dienViens = DienVien::where('trang_thai', 1)->get();
        $theLoaiPhims = TheLoaiPhim::where('trang_thai', 1)->get();
        return view('nhanVien.phims.edit', compact('phim', 'daoDiens', 'dienViens', 'theLoaiPhims'));
    }
    public function update(NhanvienPhimRequest $request, Phim $nhanvienphim)
    {
        $phim = $nhanvienphim;
        ...
        return redirect()->route('nhanvienphim.index')->with('success', 'Phim đã được cập nhật thành công.');
    }
    ...
    public function listSoftDelete()
    {
        ...
        return view('nhanVien.phims.listSoftDelete', compact('phims'));
    }
    public function softDelete($id)
    {
        $phim = Phim::findOrFail($id);
        $phim->delete();
        return redirect()->route('nhanvienphim.index')->with('success', 'Xóa mềm thành công!');
    }
    ...
    public function restore($id)
    {
        ...
        return redirect()->route('nhanVien.phim.listSoftDelete')->with('success', 'Khôi phục thành công!');
    }
    public function forceDelete($id)
    {
        ...
        return redirect()->route('nhanVien.phim.listSoftDelete')->with('success', 'Xóa vĩnh viễn thành công!');
    }
    public function destroy(Phim $phim)
    {
        $phim->trang_thai = 0;
        $phim->save();
        return redirect()->route('nhanvienphim.index')->with('success', 'Đạo Diễn đã được xóa thành công.');
    }
}
